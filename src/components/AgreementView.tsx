    ...
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {currentWOId && (
        <div className="bg-blue-50 border border-blue-100 p-3 rounded-2xl flex justify-between items-center no-print">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handlePrint} className="p-2 text-blue-600 bg-white rounded-full shadow-sm active:scale-95" title="列印 PDF"><Printer size={18}/></button>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden print:border-0 print:shadow-none">
        ...
        <div className="p-5 space-y-6 print:p-0 print:pt-1 print:space-y-1">
          {/* 修改：字體改為 print:text-[12px] */}
          <div className="grid gap-4 print:grid-cols-3 print:gap-2 print:text-[12px]">
            <div>
              <label className="text-xs font-bold text-slate-400 ml-1 mb-1 block print:text-black print:text-[12px] print:mb-0">工令編號</label>
              <input type="text" value={data.woNo || ''} onChange={e => onChange('woNo', e.target.value)} className="w-full bg-slate-50 border-0 rounded-xl p-3 font-bold text-slate-700 focus:ring-2 focus:ring-blue-500 font-mono uppercase print:p-0.5 print:border print:border-black print:bg-white print:rounded-none print:text-[12px]" placeholder="例如: Y6N10001" maxLength={8} />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-400 ml-1 mb-1 block print:text-black print:text-[12px] print:mb-0">工程名稱</label>
              <input type="text" value={data.woName || ''} onChange={e => onChange('woName', e.target.value)} className="w-full bg-slate-50 border-0 rounded-xl p-3 font-bold text-slate-700 focus:ring-2 focus:ring-blue-500 print:p-0.5 print:border print:border-black print:bg-white print:rounded-none print:text-[12px]" placeholder="輸入名稱..." />
            </div>
            <div className="print:col-span-1">
              <label className="text-xs font-bold text-slate-400 ml-1 mb-1 block print:text-black print:text-[12px] print:mb-0">承包廠商</label>
              <div className="relative">
                <select value={data.contractor || ''} onChange={e => onChange('contractor', e.target.value)} className="w-full bg-slate-50 border-0 rounded-xl p-3 font-bold text-slate-700 appearance-none print:p-0.5 print:border print:border-black print:bg-white print:rounded-none print:text-[12px]">
          ...
          <div className="print:text-[12px]">
            <h4 className="font-bold text-slate-800 mb-3 text-sm print:text-black print:text-[12px] print:mb-0.5">工程期限</h4>
            <div className="space-y-3 print:space-y-0">
              {[
                { id: '1', label: '工作天', content: <>本工程施工期限為 <input type="number" value={data.durationDays || ''} onChange={e => onChange('durationDays', e.target.value)} className="w-10 border-b-2 text-center font-bold text-blue-600 no-arrow print:text-black print:border-black print:text-[12px]"/> 工作天，需配合 <input type="text" value={data.durationCoop || ''} onChange={e => onChange('durationCoop', e.target.value)} className="w-16 border-b-2 text-center font-bold text-blue-600 print:text-black print:border-black print:text-[12px]"/> 施工。</> },
                { id: '2', label: '日曆天', content: <>需配合施工，其施工期限為配合工程完成後於 <input type="number" value={data.durationCalendarDays || ''} onChange={e => onChange('durationCalendarDays', e.target.value)} className="w-10 border-b-2 text-center font-bold text-blue-600 no-arrow print:text-black print:border-black print:text-[12px]"/> 日曆天內完成。</> },
                ...
                <div key={opt.id} className={`${data.durationOption !== opt.id ? 'print:hidden' : ''}`}>
                    ...
                    <div className="text-sm text-slate-600 leading-relaxed print:text-black print:text-[12px]">{opt.content}</div>
                    </label>
                </div>
              ))}
            </div>
          ...
          <div className="rounded-2xl border border-slate-100 overflow-hidden print:border-0">
            ...
            <div className="hidden print-only px-1 pt-0">
                 {/* 修改：字體 12px */}
                 <h4 className="font-bold mb-0.5 text-[12px]">安全確認事項：</h4>
          ...
          <div className="print:break-inside-avoid">
            {/* 修改：字體 12px */}
            <h4 className="font-bold text-slate-800 mb-3 text-sm print:text-black print:text-[12px] print:mb-1">簽名確認</h4>
              ...
              {SIGNATURE_ROLES.map(role => (
                <div key={role.id} onClick={() => !data.signatures?.[role.id] && handleSignClick(role)} className={`p-3 print:p-1 rounded-xl border-2 border-dashed transition-all cursor-pointer print:border print:border-black print:rounded-none ${data.signatures?.[role.id] ? 'border-blue-200 bg-blue-50/30 print:bg-transparent' : 'border-slate-200 hover:border-blue-400 hover:bg-slate-50'}`}>
                  <div className="flex justify-between items-center mb-2 print:mb-0 print:justify-center">
                    {/* 修改：字體 12px */}
                    <span className="text-xs font-bold text-slate-500 print:text-black print:text-[12px]">{role.label}</span>
                  ...
                  {data.signatures?.[role.id] ? (
                    <div className="flex flex-col items-center">
                      ...
                      {/* 修改：日期字體 12px */}
                      <input type="date" value={data.signatures[role.id]!.date} onClick={e => e.stopPropagation()} onChange={e => onDateChange(role.id, e.target.value)} className="text-xs border rounded px-1 mt-1 print:mt-0 text-gray-500 print:border-0 print:text-black print:text-[12px] print:w-full print:text-center" />
                    </div>
                  ) : (
